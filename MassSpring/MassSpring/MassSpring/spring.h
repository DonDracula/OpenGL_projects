...
public:
	...
	{
		Vec3 springVector = mass1->pos - mass2->pos;							//vector between the two masses
		
		float x =springVector.data[0];
		float y = springVector.data[1];
		float z = springVector.data[2];
		float r = sqrtf(x*x + y*y + z*z);
										//distance between the two masses

		Vec3 force;																//force initially has a zero value
...
/*
  class RopeSimulation: simulates a rope with point-like particles binded with springs. The springs have inner friction and normal length.
  One tip of the rope is stabilized at a point in space called "Vector3D ropeConnectionPos". This point can be  moved externally by a 
  method "void setRopeConnectionVel(Vector3D ropeConnectionVel)". 
  RopeSimulation creates air friction and a planer surface (or ground) with a normal in +y direction. RopeSimulation 
...
public:
	...
	Vec3 ropeConnectionPos;							//A point in space that is used to set the position of the 
	...
	RopeSimulation(										//a long long constructor with 11 parameters starts here
		...
		Vec3 gravitation,								//6. gravitational acceleration
		...
		) : Simulation(numOfMasses, m)					//The super class creates masses with weights m of each
	{
		...
		{
			masses[a]->pos.data[0] = a * springLength;		//Set x position of masses[a] with springLength distance to its neighbor
			masses[a]->pos.data[1] = 0;						//Set y position as 0 so that it stand horizontal with respect to the ground
			masses[a]->pos.data[2] = 0;						//Set z position as 0 so that it looks simple
		}
	...
	void solve()										//solve() is overriden because we have forces to be applied
	{
		...
		for (int a = 0; a < numOfMasses; ++a)				//Start a loop to apply forces which are common for all masses
		{
			...
			if (masses[a]->pos.data[1] < groundHeight)		//Forces from the ground are applied if a mass collides with the ground
			{
				...
				//above, we obtained a velocity which is vertical to the ground and it will be used in the absorption force
				...
				Vec3 force = Vec3(0, groundRepulsionConstant, 0) * 
					(groundHeight - masses[a]->pos.data[1]);
	...
	void setRopeConnectionVel(Vec3 ropeConnectionVel)	//the method to set ropeConnectionVel
	{

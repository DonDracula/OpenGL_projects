...
ShapeMatching ::ShapeMatching(int obj)
{
	m_iObjectNum = obj;
	m_fDt = 0.01f;

	m_vGraviity = Vec3(0.0f,-9.9f,0.0);

	m_vWindSpeed = Vec3(6.0f,0,0);
...
void ShapeMatching::initialize(void)
{
...
void ShapeMatching::AddCluster(const vector<int> &list)
{
	Cluster cl;
	cl.Node = list;
...
void ShapeMatching :: calExternalForces(double dt)
{
	for(int i = 0; i< m_iNumOfVertices;++i)
	{
		if(m_vFix[i]) continue;
		m_vVel[i] += m_vGraviity*dt+m_vWindSpeed*dt;
...
void ShapeMatching::calCollision(double dt)
{
	...
	if(m_Collision != 0){
		for(int i = 0; i < m_iNumOfVertices; ++i){
			...
			Vec3 &np = m_vNewPos[i];
			Vec3 &v = m_vVel[i];
			m_Collision(p, np, v, m_iObjectNum);
		}
	}
}

//shape matching function calculate the goal position
void ShapeMatching::shapeMatchingFun(Cluster &cl, double dt)
{
...
void ShapeMatching::integrate(double dt)
{
	double dt1 = 1.0f/dt;
//	m_vCurPos[0]=Vec3(0,0,0);
	for(int i = 0; i < m_iNumOfVertices; ++i){
...
void ShapeMatching::FixVertex(int i, const Vec3 &pos)
{
	m_vNewPos[i] = pos;
	m_vFix[i] = true;
}
...
void ShapeMatching::UnFixVertex(int i)
{
	m_vFix[i] = false;
}

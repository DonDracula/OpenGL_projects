...
namespace RXNumerical
{
	...
	static int pcgSolver2(vector< vector<T> > &poiss, vector<T> &x, vector<T> &b, 
						  ...
						  void (*bcfunc)(vector<T>&), int &max_iter, T &tol)
	{
		int i, k = 0;
		vector<T> r, p, q, s;
		T aa, bb, rqo, rqn, ps;
	...
	static int pcgSolver(vector< vector<double> > &poiss, vector<double> &x, vector<double> &b, 
						 ...
						 void (*bcfunc)(vector<double>*), int &max_iter, double &tol)
	{
		int i, k = 0;
...
//		if(err1sum < tol){ 
	...
	static int pcgSolverP(T **poiss, T *x, T *b, T **ic, int **neigh, int size_m, T dt, 
						  void (*bcfunc)(T*), int &max_iter, T &tol)
	{
		int i, k = 0;
		T *r, *p, *q, *s;
		T aa, bb, rqo, rqn, ps;
	...
	static int pcgSolverPB3(double **poiss, double *x, double *b, double **ic, int **neigh, int nx, int ny, int nz, double dt, 
						    void (*bcfunc)(double*), int &max_iter, double &tol)
	{
		int i, j, k, l = 0, idx;
		double *r, *p, *q, *s;
		double aa, bb, rqo, rqn, ps;
...
//		if(err1sum < tol){ 
	 ...
	 * @param[inout] tol 許容誤差
	 */
	static int cgSolver(void (*func)(vector<double>&, vector<double>), 
	...
	static double Newton1D(void (*funcd)(const double, double&, double&), 

 ...
 */
void rxParticleSystemBase::AddSphere(int start, RXREAL *pos, RXREAL *vel, int r, RXREAL spacing)
{
	uint index;
	if(start < 0){
 ...
 * @param[in] start 追加開始インデックス
 * @param[in] cen ボックスの中心
 * @param[in] ext ボックスの辺の長さの1/2
 * @param[in] vel 初期速度
 * @param[in] spacing パーティクル間隔
 */
void rxParticleSystemBase::AddBox(int start, Vec3 cen, Vec3 ext, Vec3 vel, RXREAL spacing)
{
	uint index;
	if(start < 0){
		index = m_uNumParticles;
 ...
 * @return 追加されたパーティクル数
 */
int rxParticleSystemBase::addParticles(int &start, rxInletLine line)
{
	...
	int n = l/(2*pr*line.spacing);	// 並べるパーティクルの数
	if(!n) return 0;
	...
	for(int j = 0; j < line.accum; ++j){
		for(int i = 0; i < n; ++i){
			...
			for(int k = 0; k < 3; ++k){
				m_hPos[DIM*index+k] = line.pos1[k]+rel[k]*(i+0.5)/n+(RX_FRAND()*2.0f-1.0f)*jitter;

/*! @file rx_gltexture.h
	...
	@brief PNG画像読み込み，書き込み，GLテクスチャ作成
...
#ifndef _RX_GL_TEXTURE_H_
#define _RX_GL_TEXTURE_H_
...
// MARK:画像の読込
//-----------------------------------------------------------------------------
static int g_iBitmapType = RX_BMP_WINDOWS_V3;
inline void SetBitmapType(int type){ g_iBitmapType = type; }


static unsigned char* ReadImageFile(const std::string &fn, int &w, int &h, int &c)
{
	// 拡張子抽出
	string ext;
...
static int WriteImageFile(const std::string &fn, unsigned char *img, int w, int h, int c, int quality = 200)
{
	// 拡張子抽出
	string ext;
	size_t pos1 = fn.rfind('.');
	...
	if(ext == "jpg" || ext == "jpeg"){
		...
		WriteBitmapFile(fn, img, w, h, c, g_iBitmapType);
	}
	else{
		return 0;
	}
 ...
 * @retval false 保存失敗
 */
static bool SaveFrameBuffer(const string &fn, int w, int h)
{
	vector<unsigned char> img_buf(w*h*3);

	glReadPixels(0, 0, w, h, GL_RGB, GL_UNSIGNED_BYTE, &img_buf[0]);

	// 上下反転
	int stride = w*3;
	for(int j = 0; j < h/2; ++j){
		for(int i = 0; i < stride; ++i){
			unsigned char tmp = img_buf[j*stride+i];
			img_buf[j*stride+i] = img_buf[(h-j-1)*stride+i];
			img_buf[(h-j-1)*stride+i] = tmp;
		}
	}

	WriteImageFile(fn, &img_buf[0], w, h, 3);
...
/*! テクスチャをビットマップで保存
	...
	@retval false 保存失敗
*/
static bool SaveTexture(const string &fn, rxTexObj2D &tex_obj, int jpeg_quality = 200)
{
	int w = tex_obj.m_iW;
	int h = tex_obj.m_iH;
	int c = 3;
	...
	for(jc = 0; jc < h; ++jc){
		for(ic = 0; ic < w; ++ic){
			...
			img_buf[idx+0] = tex_obj.m_pImage[idx];
			img_buf[idx+1] = tex_obj.m_pImage[idx+1];
			img_buf[idx+2] = tex_obj.m_pImage[idx+2];
		}
	}

	WriteImageFile(fn, &img_buf[0], w, h, c, jpeg_quality);
 ...
 */
static bool ReadTexture(const string &fn, rxTexObj2D &tex_obj)
{
	int w0, h0, c0;
	unsigned char* pimg;
 ...
 */
inline bool LoadTPNG(const string &fn, GLuint &tex_name, int &w, int &h, int &c)
{
	// MARK:LoadTPNG
	int w0, h0, c0;
	unsigned char* pimg;
...
inline bool LoadTPNG(const string &fn, GLuint &tex_name)
{
	int w, h, c;
